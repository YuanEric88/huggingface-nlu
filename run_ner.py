...
class BertCRF(BertPreTrainedModel):
    """
    Bert + CRF model to deal with sequence labeling tasks

    return: loss (optional), pred_ids, logits
    """
    ...
    def forward(self, input_ids, attention_mask=None,token_type_ids=None,
        ...
        # Avoid the logits corresponding to [CLS]
        logits = logits[:, 1:, :]
        # transpose the dimension from batch_size x sequence_len x num_labels
        # to sequence_len x batch_size x num_labels
        logits = torch.transpose(logits, 0, 1)
        pred_ids = self.crf.decode(logits)
        outputs = (logits,) + (pred_ids,)
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                log_likehood = self.crf(logits, labels, attention_mask, reduction="mean")
            else:
                log_likehood = self.crf(logits, labels)
            loss = -log_likehood
...
# How to use complete tokenizer in sequence labeling tokenizer remain explore.
# current change remove wordpiece tokenizer in BertTokenizer
MODEL_CLASSES = {
    "bert": (BertConfig, BertForTokenClassification, BertTokenizer),
...
def main():
    ...
    # Set pad_token_label_id always be the last index in the labels.txt file
